...
public class ConsultantProfileService {
    ...
    public ConsultantProfileService() {
        educationList.add(new ConsultantEducationDTO(1L, 1L, "석사", "국제경영학", "University of Canberra", "2018"));
        educationList.add(new ConsultantEducationDTO(2L, 1L, "학사", "영어영문학", "서울대학교", "2015"));

        careerList.add(new ConsultantCareerDTO(1L, 1L, "ABC 컨설팅", "선임 컨설턴트", "2018-2023", "글로벌 기업 대상 비즈니스 전략 수립"));
        careerList.add(new ConsultantCareerDTO(2L, 1L, "XYZ 교육", "영어 강사", "2015-2018", "기업 임직원 대상 비즈니스 영어 교육"));
    }
    ...
    public List<ConsultantEducationDTO> getEducation(Long profileId) {
        // 프로필 ID를 기준으로 교육 정보를 필터링
        return educationList.stream()
                .filter(edu -> edu.getProfileId().equals(profileId))
                .collect(Collectors.toList());
    }

    public List<ConsultantCareerDTO> getCareer(Long profileId) {
        // 프로필 ID를 기준으로 경력 정보를 필터링
        return careerList.stream()
                .filter(career -> career.getProfileId().equals(profileId))
                .collect(Collectors.toList());
    }
}
